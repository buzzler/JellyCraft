	...
	void UpdateReferencePoint()	{
		...
		if (this.m_ReferencePoints!=null) {
			if (Input.GetButtonDown ("Fire1")) {
				...
				JellySpriteReferencePoint minJPoint = null;

				foreach(ReferencePoint p in this.m_ReferencePoints) {
					if(p.Body2D.GetComponent<Collider2D>().OverlapPoint(mp) && (p!=m_CentralPoint)) {
						JellySpriteReferencePoint jrp = p.Body2D.GetComponent<JellySpriteReferencePoint>();
						float dist = Vector2.Distance(mp, jrp.GetAnchor());
						if (dist < minDist) {
							minDist = dist;
							minJPoint = jrp;
						}
				...
				foreach(ReferencePoint p in this.m_ReferencePoints) {
					p.Body2D.GetComponent<JellySpriteReferencePoint>().DragOff();
				}
			}
		}
	}
